...
#Safety index rule (dmin^2 - d^2 - d_dot )  could modify the parameters
        ...
        if (not is_safe):
            ...
            if len(phis_unsafe)>1:
                for i in phis_unsafe:
                    if i>phi_temp:
                        phi_temp = i
            return u, True, unavoid_collision, danger_obs, phi_temp                            
        u0 = u0.reshape(1,2)
        u = u0
        record_data['control'] = u[0]
        self.records.append(record_data)  
        phi_temp = phi
#Eventually, it return a new control u[0]   
        return u[0], False, False, danger_obs, phi_temp
