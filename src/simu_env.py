...
#Creates infinite looping environment for the vehicle and obstalces
def l2(xy0,xy1):
	ox = xy1[0]
	oy = xy1[1]
	dx = xy0[0] - xy1[0]
...
class Env(object):
	def __init__(self, display, obs_num, field, robot_state,
                    min_dist,
                    noise_sigma,
                    in_bounds,
                    goal_bounds,
	...
	def find_all_obstacle_loc(self, cx, cy):
		num = []
		states = []
		states = self.field.all_obstacle_states(self.cur_step, cx, cy, self.min_dist)
		return states